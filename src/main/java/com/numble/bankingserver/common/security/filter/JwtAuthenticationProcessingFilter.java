package com.numble.bankingserver.common.security.filter;
 ...
 *      -> 인증 성공 처리, RefreshToken을 재발급하지는 않는다.
 * 2. RefreshToken이 없고, AccessToken이 없거나 유효하지 않은 경우
 *      -> 인증 실패 처리, 403 ERROR
 * 3. RefreshToken이 있는 경우
 *      -> DB의 RefreshToken과 비교하여 일치하면 AccessToken 재발급, RefreshToken 재발급(RTR 방식) 인증 성공 처리는 하지 않고 실패 처리
 */
...
public class JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
     ...
     * 유효한 토큰이면, 액세스 토큰에서 extractUserId로 userId을 추출한 후 findByUserId()로 해당 아이디를 사용하는 유저 객체 반환
     * 그 유저 객체를 saveAuthentication()으로 인증 처리하여 인증 허가 처리된 객체를 SecurityContextHolder에 담기 그 후 다음 인증 필터로 진행
     */
